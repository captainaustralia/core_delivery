...
class Order(BaseModel):
    ...
    dispatch_point = models.ForeignKey(
        Location,
...
class Incident(BaseModel):
    ...
    employee_answer = models.TextField(
        verbose_name="Ответ работника принявшего инцидент"
    )
    incident_handler = models.ForeignKey(
        DefaultUser, on_delete=models.DO_NOTHING, verbose_name="Кто обработал инцидент"
    )
...
class DeliveryManReview(BaseModel):
    ...
    receiver = models.ForeignKey(
        DefaultUser, null=True, on_delete=models.SET_NULL, verbose_name="Получатель"
    )
    deliveryman = models.ForeignKey(
        DeliveryMan, null=True, on_delete=models.SET_NULL, verbose_name="Доставщик"
    )
...
class HistoricalOrderRecord(BaseModel):
    ...
    incident = models.ForeignKey(
        Incident, null=True, on_delete=models.SET_NULL, verbose_name="Инцидент"
    )
